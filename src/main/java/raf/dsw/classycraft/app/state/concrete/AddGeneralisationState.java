...
public class AddGeneralisationState implements State {
    private Generalization connection;
    private GeneralizationPainter p;
    ...
    public void mousePress(MouseEvent e, DiagramView diagramView, Point tP) {
        for (ClassyNode d : diagramView.getDiagram().getChildren()) {
            if (d instanceof InterClass && ((InterClass) d).contains(tP.x,tP.y)) {
                connection = new Generalization(diagramView.getDiagram());
        ...
        if (connection != null) {
            ...
            connection.setB(tP);
            p = new GeneralizationPainter(connection);
